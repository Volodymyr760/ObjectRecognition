...
namespace ObjectsRecognition.Services
{
    ...
    public class YoloScorer<T> : IDisposable where T : YoloModel
    {
        ...
        /// </summary>
        private DenseTensor<float>[] Inference(Bitmap image)
        {
            Bitmap resizedImage = new Bitmap(image, new Size(_model.Width, _model.Height));

            var inputs = new List<NamedOnnxValue> // add image as input
            {
                NamedOnnxValue.CreateFromTensor("images", ExtractPixels(resizedImage))
            };
        ...
        /// </summary>
        public List<YoloPrediction> Predict(Bitmap image)
        {
            return Suppress(ParseOutput(Inference(image), (image.Width, image.Height)));
        }
